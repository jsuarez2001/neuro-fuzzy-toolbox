...
class SONFIS():
    def __init__(self, ANFIS_trainer_instance, Ngrow, dGrow, Nsplit, eSplit, Nvanish, lVanish, max_it, validation=0, early_stopping=None, last_ANFIS_trainer=False, reset_ANFIS_trainer_early_stopping=True):
        # Hyperparameters
        self.max_it = max_it
        self.Ngrow = Ngrow
        self.dGrow = dGrow
        ...
        #ANFIS trainer
        self.ANFIS_trainer = ANFIS_trainer_instance
        self.reset_ANFIS_trainer_early_stopping = reset_ANFIS_trainer_early_stopping

        #Other attributes
        self.last_ANFIS_trainer = last_ANFIS_trainer
        ...
        #history
        self.loss_function = ANFIS_trainer_instance.loss_function
    ...
    def __call__(self, ANFISmodel, loader):
        ...
        self.callANFIS_trainer(ANFISmodel, loader)
        while(model_updated and (i < self.max_it)):
            print("\n *******ITERATION:", i+1, " ******* ")
            self.freeze_rules()
            ...
            print("\nval_accuracy:", self.val_history['accuracy'][-1])
        ...
        self.unfreeze_rules()
        if self.last_ANFIS_trainer:
            self.ANFIS_trainer.last = True
            self.callANFIS_trainer(ANFISmodel, loader)
        _ = self.obtain_metrics(ANFISmodel, train_loader, val_loader)
    ...
    def callANFIS_trainer(self, ANFISmodel, loader):
        self.ANFIS_trainer(ANFISmodel, loader, self.freezed)
        for metric in self.ANFIS_trainer.history:
            self.ANFIS_trainer.history[metric] = torch.tensor([])
        if ((self.ANFIS_trainer.early_stopping != None) & (self.reset_ANFIS_trainer_early_stopping)):
            self.ANFIS_trainer.early_stopping.reset()
    ...
    def GrowNet(self, ANFISmodel, loader):
        ...
        #stds = torch.stack([(bad_samples[rule].std(dim=0)) for rule in rules])/2
        ...
        new_consequents = 2 * torch.rand(ANFISmodel.consequents.shape[0], new_premises.size(1), ANFISmodel.input_size + 1) - 1
        ANFISmodel.set_consequents(torch.cat((ANFISmodel.consequents, new_consequents), dim=1))
    ...
    def SplitSubNetwork(self, ANFISmodel, loader):
        ...
        if isinstance(ANFISmodel.last_layer, nn.Softmax):
            mse_values = torch.stack([torch.pow((samples_output[best_rules == rule] - torch.max(ANFISmodel(samples[best_rules == rule]))), 2).mean(dim=0) for rule in unique_rules])
        else:
        ...
        for rule in list(torch.flip(unique_rules[eSplit_mask], [0]).long()): #the iteration is performed on a list with the rules in descending order
            ...
            new_consequents = torch.cat([new_consequents, 2 * torch.rand(ANFISmodel.consequents.shape[0], 2, ANFISmodel.input_size + 1) - 1], dim=1)
