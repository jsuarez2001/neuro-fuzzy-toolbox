...
from EXP_neuro_fuzzy_toolbox.func import GeneralizedBell_MF
...
class FuzzificationLayer(nn.Module):
    ...
    """
    def __init__(self, mf_distribution, membership_function=GeneralizedBell_MF, linguistic_variables=None, dtype=torch.float32):
        """
    ...
    def premises_structure(self):
        ...
        columns = pd.MultiIndex.from_product(
            [[f'x{i}' for i in range(self._input_size)], self._membership_function._params]
        )

        df = pd.DataFrame(columns=columns)

        mfs = [f'{i}' for i in range(self._max_n_mfs)]
        ...
        for i in range(self._input_size):
            ...
            for param_name in self._membership_function._params:
                ...
                df[(f'x{i}', param_name)] = pd.Series(column_data, index=mfs)
    ...
    def plot_premises(self, mf=None, input_dim=None, group_by_dim=False):
        ...
        """
        variables = [f'{self.linguistic_variables[i]}' for i in range(self._input_size)]
...
class h_FuzzificationLayer(nn.Module):
    ...
    """
    def __init__(self, input_size, num_mfs=1, membership_function=GeneralizedBell_MF, linguistic_variables=None, dtype=torch.float32):
        """
    ...
    def premises_structure(self):
        ...
        mfs = [f"{i}" for i in range(premises.shape[1])]
        ...
        for i in range(self._input_size):
            for j, param in enumerate(self._membership_function._params):
                ...
                df[(f'x{i}', param)] = pd.Series(column_data, index=mfs)
...
class rule_reduced_FuzzificationLayer(nn.Module):
    ...
    def premises_structure(self):
        ...
        mfs = [f"rule {i}" for i in range(n_mfs)]
        
        multi_columns = pd.MultiIndex.from_product(
            ([f'{self.linguistic_variables[i]}' for i in range(self._input_size)], self._membership_function._params)
        )
        ...
        for i in range(self._input_size):
            for j, param in enumerate(self._membership_function._params):
                ...
                df.loc[:, (self.linguistic_variables[i], param)] = pd.Series(column_data, index=mfs)
