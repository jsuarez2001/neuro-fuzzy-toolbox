...
class baseSONFIS(Hybrid_learning_algorithm):
...
class alt_SONFIS(SONFIS):
    ...
    def _GrowNet(self, ANFISmodel, train_loader):
        ...
        for batch_x, _ in train_loader:
            ...
            w_max = torch.cat((w_max, max_fl.values))
            x = torch.cat((x, batch_x))
            related_fuzzy_rules = torch.cat((related_fuzzy_rules, torch.nn.functional.one_hot(max_fl.indices, ANFISmodel.fuzzy_rules).bool()))
        ...
        if n_new_fuzzy_rules == 0:
            ...
            # Add new consequents    
            new_consequents = ANFISmodel._consequent_layer._consequent_function.initialize_consequents(ANFISmodel._outputs, n_new_fuzzy_rules, ANFISmodel._input_size, ANFISmodel._dtype)
            ANFISmodel.set_consequents(torch.cat((ANFISmodel.get_consequents(), new_consequents), dim=1))
    ...
    def _SplitSubNet(self, ANFISmodel, train_loader):
        ...
        if fuzzy_rules_to_split[fuzzy_rules_to_split == True].size(0) == 0:
            ...
            # New Consequents
            still_consequents = ANFISmodel.get_consequents()[:, ~fuzzy_rules_to_split, :]
